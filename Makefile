...
# help:
# help: datapipeline-tests Makefile help
# help:
...
venv:
	@rm -Rf venv
	@python3 -m venv venv --prompt datapipeline_tests
...
.PHONY: format
format:
	@black src/datapipeline_tests tests examples


# help: check-format                   - check code format compliance
.PHONY: check-format
check-format:
	@black --check src/datapipeline_tests tests examples
...
.PHONY: check-types
check-types:
	@cd src; mypy -p datapipeline_tests --ignore-missing-imports
...
.PHONY: docs
docs: coverage
	@cd docs; rm -rf source/api/datapipeline_tests*.rst source/api/modules.rst build/*
	@cd docs; make html
...
.PHONY: dist-test
dist-test: dist
	@cd dist && ../tests/test-dist.bash ./datapipeline_tests-*-py3-none-any.whl


# help: dist-upload                    - upload a wheel distribution package
.PHONY: dist-upload
dist-upload:
	@twine upload dist/datapipeline_tests-*-py3-none-any.whl
