...
The datapipeline-tests project implements a regression
...
not broken existing functionality. The datapipeline-tests
...
.. code-block:: console
    ...
    (datapipeline_tests) $ make test

To run tests verbosely use:

.. code-block:: console

    (datapipeline_tests) $ make test-verbose
...
``datapipeline_tests`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``datapipeline_tests`` package can be found.

.. code-block:: console

    (datapipeline_tests) $ cd tests
    (datapipeline_tests) $ python -m unittest

Individual unit tests can be run also.

.. code-block:: console

    (datapipeline_tests) $ python -m test_version
...
.. code-block:: console
    ...
    (datapipeline_tests) $ make coverage
...
.. code-block:: console
    ...
    (datapipeline_tests) $ make format
...
.. code-block:: console
    ...
    (datapipeline_tests) $ make sort-imports
...
.. code-block:: console
    ...
    (datapipeline_tests) $ make check-static-analysis
...
.. code-block:: console
    ...
    (datapipeline_tests) $ make check-lint
...
.. code-block:: console
    ...
    (datapipeline_tests) $ make check-types
...
.. code-block:: console
    ...
    (datapipeline_tests) $ make check-docs
...
.. code-block:: console
    ...
    (datapipeline_tests) $ make serve-docs
...
- Create and push a repo tag to Github. As a convention use the package
  ...
  - This will trigger Github to create a release at:
    ...
    ::
        ...
        https://github.com/{username}/datapipeline_tests/releases/{tag}
...
- Test the release distribution. This involves creating a virtual environment,
  ...
  .. code-block:: console
      ...
      (datapipeline_tests) $ make dist-test

- Upload the release to PyPI using

  .. code-block:: console

      (datapipeline_tests) $ make dist-upload

  The package should now be available at https://pypi.org/project/datapipeline_tests/
